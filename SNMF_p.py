...
def train(lrD,layerNum, lr, train_data, test_data, nrtrain, A0, S0, X, A, s, SNR):
    ...
    print(output_data)
    return meanDistance.item(), rmse, p
if __name__ == '__main__':
    ...
    for snr in snr_list:
        ...
        # print(f"lr={lr},  lrD={lrD}")
            sad, rmse, sparse_p = train(lrD=lrD, lr=lr, layerNum=layerNum, train_data=train_data, test_data=train_labels, nrtrain=nrtrain, A0=A0, S0=S0,
                X=X, A=A, s=s.T, SNR='30dB')
            # sad_list.append(sad)
            # rmse_list.append(rmse)
            # k += 1
            save_p(sparse_p, layerNum, 'graphs/synth/p_test_snmf_SNR' + snr + '_layerNum' + str(layerNum) + '.mat')
